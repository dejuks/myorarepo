...
3. **Module admins can create platform user accounts.** Each module's dashboard (`web/src/pages/ModuleRolesPage.tsx`) now includes a "Create user" action, available to the same module-local top role (or platform ADMIN) that manages member roles there. It creates a real, full platform account — not a module-scoped one, since no module service stores user profiles — via the same two-step flow the public self-registration page uses (`POST /users` to create the profile in `user-service`, then `POST /auth/register` to create credentials in `auth-service` for the same id). The newly created user is immediately selected in the dashboard so the module admin can assign it a role in their module in the same flow.
