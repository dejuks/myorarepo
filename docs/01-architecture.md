...
- **Endpoints.** `GET/POST /api/v1/wiki/articles`, `GET/PUT /api/v1/wiki/articles/:slug`, `GET /api/v1/wiki/articles/:slug/revisions`, `GET /api/v1/wiki/articles/:slug/revisions/:revisionId`. Reads have no `requireAuth` (matches the gateway's existing `requiresAuth: false` setting for the `/api/v1/wiki` prefix); writes require only a valid login.
- **Editing gate.** No separate "Registered Editor" role check in Phase 1 — any authenticated platform account can create/edit, mirroring real Wikipedia's UX where logging in is the only bar to editing. This is also a deliberate way to sidestep a platform-wide gap (module-local roles such as `BUREAUCRAT` never reach the JWT `roles` claim — see `wiki-service`'s README) until Phase 2's moderation workflows need it fixed.
