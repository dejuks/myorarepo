...
export function createApp(): Application {
  ...
  // see services/gateway/src/proxy/proxy-router.ts), so these must be
  // mounted at the exact prefix reserved for this service in
  // services/gateway/src/config/service-registry.ts.
  app.use('/api/v1/researchers', roleRouter);
  app.use('/api/v1/researchers', memberRoleRouter);
