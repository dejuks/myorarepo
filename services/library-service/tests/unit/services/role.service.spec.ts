...
describe('RoleService', () => {
  ...
  beforeEach(() => {
    ...
    roleRepo.seed(['MEMBER', 'LIBRARY_MANAGER']);
    roleService = new RoleService(roleRepo);
  });

  it('lists all roles', async () => {
    const roles = await roleService.listRoles();
    expect(roles.map((r) => r.name).sort()).toEqual(['LIBRARY_MANAGER', 'MEMBER']);
  });

  it('creates a new custom role', async () => {
    const role = await roleService.createRole({ name: 'ACQUISITIONS_CLERK', description: 'Handles new acquisitions paperwork' });
    expect(role.name).toBe('ACQUISITIONS_CLERK');
    expect(role.isSystem).toBe(false);
  });

  it('rejects creating a role with a duplicate name', async () => {
    await expect(roleService.createRole({ name: 'MEMBER' })).rejects.toMatchObject({ statusCode: 409 });
  });

  it('rejects deleting a system role', async () => {
    const systemRole = (await roleRepo.listAll()).find((r) => r.name === 'LIBRARY_MANAGER')!;
