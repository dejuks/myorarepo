...
describe('bootstrapModuleAdmin', () => {
  ...
  it('assigns both MEMBER and LIBRARY_MANAGER to the deterministic bootstrap user id', async () => {
    const { roleRepo, userRoleRepo } = makeRepos();
    ...
    expect(await userRoleRepo.listRoleNamesForUser(userId)).toEqual(expect.arrayContaining(['MEMBER', 'LIBRARY_MANAGER']));
  });

  it('is idempotent: does not duplicate role assignments across multiple runs', async () => {
    const { roleRepo, userRoleRepo } = makeRepos();
    ...
    const userId = computeBootstrapUserId('admin@ora.local');
    expect(userRoleRepo.rows.filter((r) => r.userId === userId)).toHaveLength(2); // MEMBER + LIBRARY_MANAGER, not 4
  });
  ...
  it('is case-insensitive and trims the configured email', async () => {
    const { roleRepo, userRoleRepo } = makeRepos();
