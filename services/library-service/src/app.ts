...
export function createApp(): Application {
  ...
  // Mounted at the FULL gateway prefix (/api/v1/library), not just /api/v1
  // — the gateway forwards req.originalUrl unchanged, so these routes must
  // literally start with the module's gateway prefix.
  app.use('/api/v1/library', roleRouter);
  app.use('/api/v1/library', memberRoleRouter);
