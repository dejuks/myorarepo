...
Six system roles (`LIBRARY_MANAGER`, `DIGITAL_LIBRARIAN`, `LIBRARIAN`, `CATALOGER`, `INVENTORY_MANAGER`, `MEMBER`) are seeded by the initial migration. `LIBRARY_MANAGER` is this module's top role — the only role allowed to create/delete custom roles and assign/revoke roles for other members. `MEMBER` is the base role, auto-assigned alongside `LIBRARY_MANAGER` when the platform super-admin is bootstrapped.
