...
describe('RoleService', () => {
  ...
  it('lists all roles', async () => {
    ...
    expect(roles.map((r) => r.name).sort()).toEqual(['ASSOCIATE_EDITOR', 'AUTHOR', 'EDITOR_IN_CHIEF', 'JOURNAL_MANAGER', 'REVIEWER']);
  });

  it('creates a new custom role', async () => {
    const role = await roleService.createRole({ name: 'COPY_EDITOR', description: 'Handles copyediting and formatting' });
    expect(role.name).toBe('COPY_EDITOR');
    expect(role.isSystem).toBe(false);
  });

  it('rejects creating a role with a duplicate name', async () => {
    await expect(roleService.createRole({ name: 'AUTHOR' })).rejects.toMatchObject({ statusCode: 409 });
  });

  it('rejects deleting a system role', async () => {
    const systemRole = (await roleRepo.listAll()).find((r) => r.name === 'JOURNAL_MANAGER')!;
