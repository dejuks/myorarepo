...
### Extended profile fields and expiring role assignments

The `User` entity carries a few extra demographic/contact fields beyond the original core set — `gender` (enum: `MALE`/`FEMALE`/`OTHER`/`PREFER_NOT_TO_SAY`), `dateOfBirth` (date only, no time component), `address`, `country`, `region`, `city`, and `timezone` (a free-text IANA name, e.g. `Africa/Addis_Ababa`, not validated against the tz database). All seven are optional/nullable and are only ever written via `PATCH /users/:id` (`UpdateProfileDto`) — there's no separate endpoint for them, matching the existing pattern for `avatarUrl`/`bio`/`phone`. Migration: `1758000300000-AddProfileFieldsAndRoleExpiration`.

Role assignments (`POST /users/:id/roles`, `AssignRoleDto`) accept an optional `expiresAt` (ISO 8601 timestamp, must be in the future) for a temporary/expiring grant — useful for things like a 30-day trial role or a time-boxed elevated permission. Once `expiresAt` passes, the assignment isn't deleted; it's simply excluded going forward by `UserRoleAssignmentRepository.listRoleNamesForUser`/`isAssigned` (`WHERE expires_at IS NULL OR expires_at > NOW()`), the same query both `UserService.attachRoles` and `PermissionService.userHasPermission` read from on every request. So an expired role stops granting its permissions and stops showing up in the user's role list automatically, on the very next request — no cleanup job, cron, or re-login required, reusing the exact live-lookup mechanism the permission system already relies on. Re-assigning an already-held role (e.g. to renew or change its expiration) updates the existing row in place rather than erroring.
