...
Any authenticated platform account can create and edit articles — there is no separate "become an editor" step, matching real Wikipedia's UX, and `REGISTERED_EDITOR` is treated as synonymous with "logged in." Moving an article through the review/approval workflow (submit/review/publish/archive) is the part that's actually gated — see "Review & approval workflow" and "The JWT-roles gap, resolved" above.
