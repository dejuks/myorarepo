...
2. In **auth-service** Swagger (`:4001/api-docs`), call `POST /auth/register` with the **same id** as `userId`:
   ...
   ```
3. **Login will now fail** with a 403 ("Please verify your email before logging in...") until you verify — accounts start `PENDING_VERIFICATION` and stay that way until the link is clicked (see `docs/01-architecture.md` "Email verification"). Check the **notification-service** logs (`docker compose logs -f notification-service`) for the `EMAIL_VERIFICATION_REQUESTED` entry the console email provider logged — its `body` field contains the full verification URL, e.g. `http://localhost:3000/verify-email?token=<rawToken>`. Copy the `token` query-param value.
